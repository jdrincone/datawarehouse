...
def main():
    ...
    # 1.
    logger.info("1. ¿Cuáles son las tiendas con compras de al menos 100 clientes diferentes?")
    df = show_ask_sql(AskSql.tiendas_comp, cursor)
    logger.info(f'Solution: {df}')

    # 2.
    logger.info("""2. ¿Cuáles son los 5 barrios donde la mayor cantidad de clientes
                    únicos realizan compras en tiendas tipo 'Tienda Regional'?""")
    df = show_ask_sql(AskSql.barrios_tienda, cursor)
    logger.info(f'Solution: {df}')

    # 3.
    logger.info("""¿Cuáles son las tiendas y sus barrios dónde las compras en el último 
